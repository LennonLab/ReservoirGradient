...
hill.water <- as_tibble(divestim.df) %>% 
...
alpha.fig <- hill.water %>% filter(type == "water", order == 0) %>% 
  ...
  annotate("text", x = 375, y = yposDNA + 300, 
           label = "Total", size = 5) +
  annotate(geom = "text", x = 0, y = 2500, hjust = 1, vjust = 1, size = 5,
...
predict(model.terr, newdata = data.frame(distance = 400, molecule = c("RNA", "DNA")))
pander(model.terr)
